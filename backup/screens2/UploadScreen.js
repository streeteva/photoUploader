...
import { TouchableOpacity, Button, Image, View, Text, ScrollView, StyleSheet, Alert } from 'react-native';
...
export default function UploadScreen({ userId,token, onLogout }) {
  ...
  const uploadImages = async () => {
    ...
    formData.append('row1Selection', row1Selection);
    ...
    images.forEach((img, index) => {
      ...
      try {
        const res = await fetch('https://69d6-202-12-94-240.ngrok-free.app/uploads', {
          method: 'POST',
          body: formData,
          headers: {
        ...
        Alert.alert(`Upload failed for one or more images`)
  ...
  <View style={styles.checkboxContainer}>
    ...
    />
    <Text style={styles.label}>nonInfected</Text>
  </View>
</View>
<View style={styles.checkboxRow}>
  <View style={styles.checkboxContainer}>
  ...
  <View style={styles.checkboxContainer}>
    ...
    />
    <Text style={styles.label}>nonInfrared</Text>
  </View>
</View>
...
const styles = StyleSheet.create({
  ...
  buttonRow: {
    ...
    gap: 0, // if using React Native 0.71+ or Expo SDK 48+
    marginTop: 50,
  },
  icon: {
    marginRight: 10,
  ...
  checkboxRow: {
    flexDirection: 'row',
    marginLeft: 28,
    gap: 30, // use padding/margin if gap is unsupported
  },
  checkboxContainer: {
    flexDirection: 'row',
    alignItems: 'center',
